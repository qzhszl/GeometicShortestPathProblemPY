...
def generate_r2SRGG_mothernetwork(Edindex, betaindex):
    ...
    N = 10000
    ED_list = [256, 512, 1024]  # Expected degrees
    ED = ED_list[Edindex]
    print("ED:", ED)
    ...
    FileNetworkName = "/home/qzh/network/NetworkOriginalED{EDn}Beta{betan}Noise{no}.txt".format(
        EDn=ED, betan=beta, no=noise_amplitude)
    nx.write_edgelist(G, FileNetworkName)

    FileNetworkCoorName = "/home/qzh/network/CoorED{EDn}Beta{betan}Noise{no}mothernetwork.txt".format(
        EDn=ED, betan=beta, no=noise_amplitude)
...
def generate_r2SRGG_withdiffinput_clu(ED, beta, noise_amplitude):
    ...
    Coorx = []
    Coory = []
    FileNetworkCoorName = "/home/qzh/network/CoorED{EDn}Beta{betan}Noise0mothernetwork.txt".format(
        EDn=ED, betan=beta)
    ...
    Coorx = add_uniform_random_noise_to_coordinates_R2(Coorx, noise_amplitude)
    Coory = add_uniform_random_noise_to_coordinates_R2(Coory, noise_amplitude)
    FileNetworkCoorName = "/home/qzh/network/CoorwithNoiseED{EDn}Beta{betan}Noise{no}.txt".format(
        EDn=ED, betan=beta, no=noise_amplitude)
    with open(FileNetworkCoorName, "w") as file:
        for data1, data2 in zip(Coorx, Coory):
            file.write(f"{data1}\t{data2}\n")
    ...
    for ExternalSimutime in range(100):
        ...
        FileNetworkName = "/home/qzh/network/NetworkwithNoiseED{EDn}Beta{betan}Noise{no}PYSimu{ST}.txt".format(
...
def predict_geodistance_Vs_reconstructionRGG_SRGG_withnoise_SP_R2_clu(ED, beta, noise_amplitude, ExternalSimutime):
    ...
    for nodepair in unique_pairs:
        ...
        Recall_RGG_nodepair.append(recall_RGG)
        print(
            fr"inputpara:ED:{ED},beta:{beta},noise:{noise_amplitude},simu:{ExternalSimutime}, PreRGG:{precision_RGG}, RecallRGG:{recall_RGG}",
            flush=True)
        if precision_RGG != recall_RGG:
            print("predict true", SPNodeList_RGG)
            print("ground truth", SPNodelist)
        ...
        Recall_Geodis_nodepair.append(recall_Geo)
        print(
            fr"inputpara:ED:{ED},beta:{beta},noise:{noise_amplitude},simu:{ExternalSimutime}, PreGeo:{precision_Geo},RecallGeo:{recall_Geo}",
            flush=True)
        if precision_Geo != recall_Geo:
            print("predict true", Top100closednode)
            print("ground truth", SPNodelist)
        ...
        # Recall_SRGG_nodepair.append(recall_SRGG)
        # print(
        #     fr"inputpara:ED:{ED},beta:{beta},noise:{noise_amplitude},simu:{ExternalSimutime}, PRESRGG:{Precision_SRGG_nodepair},recallSRGG:{Recall_SRGG_nodepair}",
        #     flush=True)
        # if precision_SRGG != recall_SRGG:
        #     print("predict true", SPNodeList_SRGG)
        #     print("ground truth", SPNodelist)
...
def predict_randomselectnode_ub(ED, beta, noise_amplitude, ExternalSimutime):
    ...
    Precision_random_nodepair = []
    Recall_random_nodepair = []
    # Precision_RGG_nodepair = []  # save the precision_RGG for each node pair, we selected 100 node pair in total
    # Recall_RGG_nodepair = []  # we selected 100 node pair in total
    # Precision_SRGG_nodepair = []
    # Recall_SRGG_nodepair = []
    ...
    for nodepair in unique_pairs:
        ...
        random_node_list = random.sample(range(N), Predicted_truecase_num)
        spnode_predictdebyrandom = np.zeros(N)
        spnode_predictdebyrandom[random_node_list] = 1  # True cases
        precision_Geo = precision_score(Label_med, spnode_predictdebyrandom)
        recall_Geo = recall_score(Label_med, spnode_predictdebyrandom)

        # Store precision and recall values
        Precision_random_nodepair.append(precision_Geo)
        Recall_random_nodepair.append(recall_Geo)
        # print(
        #     fr"inputpara:ED:{ED},beta:{beta},noise:{noise_amplitude},simu:{ExternalSimutime}, PreGeo:{Precision_random_nodepair}",
        #     flush=True)
    ...
    print(
        fr"inputpara:ED:{ED},beta:{beta},noise:{noise_amplitude},simu:{ExternalSimutime}, Mean Pre Geodistance:{np.mean(Precision_random_nodepair)}",
        flush=True)
    print(
        fr"inputpara:ED:{ED},beta:{beta},noise:{noise_amplitude},simu:{ExternalSimutime}, Mean Recall Geodistance:{np.mean(Recall_random_nodepair)}",
        flush=True)



def predict_by_random_selct():
    """"
