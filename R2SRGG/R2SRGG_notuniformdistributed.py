...
def R2SRGG_Gaussian(N, avg, beta, rg, Coorx=None, Coory=None, SaveNetworkPath=None):
    """
    ...
    if Coorx is not None and Coory is not None:
        ...
        xx, yy, _, _ = generate_gaussian_clusters(
            n=N,
            K=10, # NUMBER OF CLUSTERS
            sigma_range=(0.03, 0.2),
            seed=None
        )
...
def generate_gaussian_clusters(n, K, box=(0,1,0,1),
                               sigma_range=(0.02, 1),
                               seed=None):
    """
    n: number of nodes
    ...
    sigma_range: range of std deviation
    """
    ...
    centers = np.column_stack([
        np.random.uniform(xmin + margin, xmax - margin, K),
        np.random.uniform(ymin + margin, ymax - margin, K)
    ])
...
def check_realdegree_vs_expecteddegree():
    ...
    for network_size in [1000,10000,100000]:
        G,_,_ = R2SRGG_Gaussian(network_size, avg, 6, rg)
