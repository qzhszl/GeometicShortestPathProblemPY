...
def predict_center_vs_side_geodistance_Vs_reconstructionRGG_SRGG_withnoise_SP_R2_clu(Edindex, betaindex, noiseindex, ExternalSimutime):
    ...
    if ED ==4:
        filename_selecetednodepair = "/home/zqiu1/network/SelecetedNodepairED{EDn}Beta{betan}Noise{no}h33.txt".format(
            Nn=N, EDn=ED, betan=beta, ST=ExternalSimutime)
    else:
        filename_selecetednodepair = "/home/zqiu1/network/SelecetedNodepairED{EDn}Beta{betan}Noise{no}h13.txt".format(
            Nn=N, EDn=ED, betan=beta, ST=ExternalSimutime)
    selected_node_pairs = np.loadtxt(filename_selecetednodepair, dtype="%i")
    nodepair_num = 50
    ...
    precision_RGG_Name = "/home/zqiu1/data/PrecisionRGGED{EDn}Beta{betan}Noise{no}PYSimu{ST}_center.txt".format(
        EDn=ED, betan=beta, no=noise_amplitude, ST=ExternalSimutime)
    np.savetxt(precision_RGG_Name, Precision_RGG_nodepair_center)

    recall_RGG_Name = "/home/zqiu1/data/RecallRGGED{EDn}Beta{betan}Noise{no}PYSimu{ST}_center.txt".format(
        EDn=ED, betan=beta, no=noise_amplitude, ST=ExternalSimutime)
    np.savetxt(recall_RGG_Name, Recall_RGG_nodepair_center)

    precision_Geodis_Name = "/home/zqiu1/data/PrecisionGeodisED{EDn}Beta{betan}Noise{no}PYSimu{ST}_center.txt".format(
        EDn=ED, betan=beta, no=noise_amplitude, ST=ExternalSimutime)
    np.savetxt(precision_Geodis_Name, Precision_Geodis_nodepair_center)

    recall_Geodis_Name = "/home/zqiu1/data/RecallGeodisED{EDn}Beta{betan}Noise{no}PYSimu{ST}_center.txt".format(
        EDn=ED, betan=beta, no=noise_amplitude, ST=ExternalSimutime)
    np.savetxt(recall_Geodis_Name, Recall_Geodis_nodepair_center)

    precision_SRGG_Name = "/home/zqiu1/data/PrecisionSRGGED{EDn}Beta{betan}Noise{no}PYSimu{ST}_center.txt".format(
        EDn=ED, betan=beta, no=noise_amplitude, ST=ExternalSimutime)
    np.savetxt(precision_SRGG_Name, Precision_SRGG_nodepair_center)

    recall_SRGG_Name = "/home/zqiu1/data/RecallSRGGED{EDn}Beta{betan}Noise{no}PYSimu{ST}_center.txt".format(
        EDn=ED, betan=beta, no=noise_amplitude, ST=ExternalSimutime)
    np.savetxt(recall_SRGG_Name, Recall_SRGG_nodepair_center)


    precision_RGG_Name_side = "/home/zqiu1/data/PrecisionRGGED{EDn}Beta{betan}Noise{no}PYSimu{ST}_side.txt".format(
        EDn=ED, betan=beta, no=noise_amplitude, ST=ExternalSimutime)
    np.savetxt(precision_RGG_Name_side, Precision_RGG_nodepair_side)

    recall_RGG_Name_side = "/home/zqiu1/data/RecallRGGED{EDn}Beta{betan}Noise{no}PYSimu{ST}_side.txt".format(
        EDn=ED, betan=beta, no=noise_amplitude, ST=ExternalSimutime)
    np.savetxt(recall_RGG_Name_side, Recall_RGG_nodepair_side)

    precision_Geodis_Name_side = "/home/zqiu1/data/PrecisionGeodisED{EDn}Beta{betan}Noise{no}PYSimu{ST}_side.txt".format(
        EDn=ED, betan=beta, no=noise_amplitude, ST=ExternalSimutime)
    np.savetxt(precision_Geodis_Name_side, Precision_Geodis_nodepair_side)

    recall_Geodis_Name_side = "/home/zqiu1/data/RecallGeodisED{EDn}Beta{betan}Noise{no}PYSimu{ST}_side.txt".format(
        EDn=ED, betan=beta, no=noise_amplitude, ST=ExternalSimutime)
    np.savetxt(recall_Geodis_Name_side, Recall_Geodis_nodepair_side)

    precision_SRGG_Name_side = "/home/zqiu1/data/PrecisionSRGGED{EDn}Beta{betan}Noise{no}PYSimu{ST}_side.txt".format(
        EDn=ED, betan=beta, no=noise_amplitude, ST=ExternalSimutime)
    np.savetxt(precision_SRGG_Name_side, Precision_SRGG_nodepair_side)

    recall_SRGG_Name_side = "/home/zqiu1/data/RecallSRGGED{EDn}Beta{betan}Noise{no}PYSimu{ST}_side.txt".format(
        EDn=ED, betan=beta, no=noise_amplitude, ST=ExternalSimutime)
