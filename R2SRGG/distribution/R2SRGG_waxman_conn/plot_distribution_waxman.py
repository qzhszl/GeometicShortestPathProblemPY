...
def load_large_network_results_maxminave(N, ED, beta,eta):
    ...
    for N in [N]:
        ...
        Nn=N, EDn=ED, betan=beta,etan=eta, ST=ExternalSimutime)
                    ...
                    deviation_vec_name = "D:\\data\\geometric shortest path problem\\EuclideanSRGG\\max_min_ave_ran_deviation\\largenetwork\\ave_deviation_N{Nn}ED{EDn}Beta{betan}eta{etan}Simu{ST}.txt".format(
        Nn=N, EDn=ED, betan=beta,etan=eta, ST=ExternalSimutime)
                    ave_deviation_for_a_para_comb_10times = np.loadtxt(deviation_vec_name)
                    ave_deviation_vec.extend(ave_deviation_for_a_para_comb_10times)

                    max_deviation_name = "D:\\data\\geometric shortest path problem\\EuclideanSRGG\\max_min_ave_ran_deviation\\largenetwork\\max_deviation_N{Nn}ED{EDn}Beta{betan}eta{etan}Simu{ST}.txt".format(
        Nn=N, EDn=ED, betan=beta,etan=eta, ST=ExternalSimutime)
                    max_deviation_for_a_para_comb_10times = np.loadtxt(max_deviation_name)
                    max_deviation_vec.extend(max_deviation_for_a_para_comb_10times)

                    min_deviation_name = "D:\\data\\geometric shortest path problem\\EuclideanSRGG\\max_min_ave_ran_deviation\\largenetwork\\min_deviation_N{Nn}ED{EDn}Beta{betan}eta{etan}Simu{ST}.txt".format(
        Nn=N, EDn=ED, betan=beta,etan=eta, ST=ExternalSimutime)
                    min_deviation_for_a_para_comb_10times = np.loadtxt(min_deviation_name)
                    min_deviation_vec.extend(min_deviation_for_a_para_comb_10times)

                    ave_baseline_deviation_name = "D:\\data\\geometric shortest path problem\\EuclideanSRGG\\max_min_ave_ran_deviation\\largenetwork\\ave_baseline_deviation_N{Nn}ED{EDn}Beta{betan}eta{etan}Simu{ST}.txt".format(
        Nn=N, EDn=ED, betan=beta,etan=eta, ST=ExternalSimutime)
                    ave_baseline_deviation_for_a_para_comb_10times = np.loadtxt(ave_baseline_deviation_name)
                    ran_deviation_vec.extend(ave_baseline_deviation_for_a_para_comb_10times)
            except FileNotFoundError:
