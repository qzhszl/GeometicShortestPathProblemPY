...
@Author: Zhihao Qiu
@Date: 04/04/2026
This .M is the final version for the real stretch and the approximation of stretch
"""
import os
import random
import re
...
def plot_stretch_approxstretch_with_avg():
    # the x-axis is the input average degree
    N = 10000
    ...
    kvec2=[2.2, 3.0, 3.8,  6.0,  8, 10, 16, 27, 44, 72, 118, 193, 316, 518, 848, 1389,
                2276,
                3727, 6105,
                9999, 16479, 27081, 44767, 73534, 121205, 199999]
    real_ave_degree_vec2, _, _, _, _, _, _, ave_L_vec2, std_L_vec2 = load_large_network_results_dev_vs_avg_beta128(
        N, beta, kvec2, False)



    lengend = [r"$\langle S \rangle$", r"$\langle d_l \rangle \langle h \rangle$"]
    fig, ax = plt.subplots(figsize=(9, 6))
    ...
    plt.errorbar(real_ave_degree_vec, ave_L_vec, yerr=std_L_vec, linestyle="--", linewidth=3, elinewidth=1, capsize=5, marker='o', markersize=16,
                 label=lengend[0], color=colors[0])

    plt.errorbar(real_ave_degree_vec2, ave_L_vec2, yerr=std_L_vec2, linestyle="--", linewidth=3, elinewidth=1, capsize=5,
                 marker='o', markersize=16,
    ...
    ax.text(-0.34, 1.15, fr'(b) $N = 10^4$, $\beta = 128$',
...
def plot_stretch_approxstretch_with_avg_diffbeta():
    # Figure appendix
    # the x-axis is the input average degree
    # realL
    N = 10000
    ...
    beta = 64   # beta_vec = [2.5,3,4,8,128]
    folder_name = "D:\\data\\geometric shortest path problem\\EuclideanSRGG\\deviaitonvsSPgeometriclength\\approxLrealdiff\\fordiffbeta\\"
    ...
    kvec_dict = {
        ...
        64: [0.6,0.9,1.2,1.6,2.4, 3.4, 4.6,  6.0,  8.0,  10.0, 16.0, 27.0, 44.0, 72.0,
              118.0, 193.0, 316.0, 518.0, 848.0, 1389.0, 2276.0, 3727.0, 6105.0, 9999.0, 16479.0, 27081.0, 44767.0, 73534.0, 220000.0,
              328000.0,
              888636.0],
    ...
    real_ave_degree_vec2, _, _, _, _, _, _, ave_L_vec2, std_L_vec2 = load_large_network_results_dev_vs_avg_approxLrealfordiffbeta(
    ...
    plt.errorbar(real_ave_degree_vec2, ave_L_vec2, yerr=std_L_vec2, linestyle="--", linewidth=3, elinewidth=1,
                 capsize=5,
                 marker='o', markersize=16,
                 label=lengend[1], color=colors[3])
    ...
    plt.legend(fontsize=26, loc=(0.6, 0.05))
    plt.tick_params(axis='both', which="both", length=6, width=1)
    picname = f"D:\\data\\geometric shortest path problem\\EuclideanSRGG\\deviaitonvsSPgeometriclength\\L_vs_realavg_beta{beta}.svg"
    plt.savefig(
        picname,
        format="svg",
        bbox_inches='tight',  # 紧凑边界
