...
def load_10000nodenetwork_results_beta_fixdistance(beta,geodesic_distance_AB):
    """
    the function is the same as load_10000nodenetwork_results but the results are based on clean data
    :param beta:
    :return:
    ...
    # real_ave_degree_Name = "D:\\data\\geometric shortest path problem\\EuclideanSRGG\\max_min_ave_ran_deviation\\largenetwork\\10000node\\real_ave_degree_Beta{betan}.txt".format(betan=beta)
    # np.savetxt(real_ave_degree_Name, real_ave_degree_vec)
    ave_deviation_Name = "D:\\data\\geometric shortest path problem\\EuclideanSRGG\\max_min_ave_ran_deviation\\GivenGeodistance\\ave_deviation_beta{betan}Dis{geodesic_distance_AB}.txt".format(betan=beta,geodesic_distance_AB = geodesic_distance_AB)
    np.savetxt(ave_deviation_Name, ave_deviation_vec)
    std_deviation_Name = "D:\\data\\geometric shortest path problem\\EuclideanSRGG\\max_min_ave_ran_deviation\\GivenGeodistance\\std_deviation_beta{betan}Dis{geodesic_distance_AB}.txt".format(betan=beta,geodesic_distance_AB = geodesic_distance_AB)
    np.savetxt(std_deviation_Name, std_deviation_vec)
    return ave_deviation_vec,std_deviation_vec, exemptionlist

def plot_local_optimum_with_realED_diffbeta_fix_distance(Geodistance_index):
    """
    the x-axis is the expected degree, the y-axis is the average deviation, different line is different c_G
    inset is the min(average deviation) vs c_G
    the x-axis is real (approximate) degree
    ...
    lengend = [r"$\beta=2.2$",r"$\beta=2^2$",r"$\beta=2^3$",r"$\beta=2^4$",r"$\beta=2^5$",r"$\beta=2^7$"]
    fig, ax = plt.subplots(figsize=(12, 8))
    ...
    picname = "D:\\data\\geometric shortest path problem\\EuclideanSRGG\\max_min_ave_ran_deviation\\LocalOptimum_dev_vs_avg_diffbeta_distance{dis}.pdf".format(
        dis=geodesic_distance_AB)
    plt.savefig(picname,format='pdf', bbox_inches='tight', dpi=600)
    plt.show()
    plt.close()
...
if __name__ == '__main__':
    ...
    """
    plot_local_optimum_with_realED_diffbeta_fix_distance_clean for different geodistance index: 
    """
    # # load data: the first time use it
    # #_______________________________________________________________________
