...
def distance_insmallSRGG_realavg(N, ED, beta, ExternalSimutime):
    """
    :param N:
    :param ED:
    :param beta:
    ...
    simu_times = 100
    for simu_index in range(simu_times):
        G, Coorx, Coory = generate_SRGG_realavg(N,ED,beta)
        try:
            real_avg = 2 * nx.number_of_edges(G) / nx.number_of_nodes(G)
        except:
            flag = 0
        ...
        for node_pair in unique_pairs:
            ...
            if SPnodenum>0:
                ...
                # ave_baseline_deviation.append(np.mean(baseline_deviations_for_a_nodepair))
        count_vec.append(count)
    file_folder = "D:\\data\\geometric shortest path problem\\EuclideanSRGG\\max_min_ave_ran_deviation\\smallnetwork\\realavg\\"
    # real_ave_degree_name = file_folder+"real_ave_degree_N{Nn}ED{EDn}Beta{betan}Simu{ST}.txt".format(
    #     Nn = N, EDn=ED, betan=beta, ST=ExternalSimutime)
    # np.savetxt(real_ave_degree_name, real_ave_degree)
    # LCC_num_name = file_folder+"LCC_num_N{Nn}ED{EDn}Beta{betan}Simu{ST}.txt".format(
    #     Nn = N, EDn=ED, betan=beta, ST=ExternalSimutime)
    # np.savetxt(LCC_num_name, LCC_num, fmt="%i")
    # clustering_coefficient_name = file_folder+"clustering_coefficient_N{Nn}ED{EDn}Beta{betan}Simu{ST}.txt".format(
    #     Nn = N, EDn=ED, betan=beta, ST=ExternalSimutime)
    # np.savetxt(clustering_coefficient_name, clustering_coefficient)
    # # For each node pair:
    ave_deviation_name = file_folder+"ave_deviation_N{Nn}ED{EDn}Beta{betan}Simu{ST}.txt".format(
        Nn = N, EDn=ED, betan=beta, ST=ExternalSimutime)
    np.savetxt(ave_deviation_name, ave_deviation)
    # max_deviation_name = file_folder+"max_deviation_N{Nn}ED{EDn}Beta{betan}Simu{ST}.txt".format(
    #     Nn = N, EDn=ED, betan=beta, ST=ExternalSimutime)
    # np.savetxt(max_deviation_name, max_deviation)
    # min_deviation_name = file_folder+"min_deviation_N{Nn}ED{EDn}Beta{betan}Simu{ST}.txt".format(
    #     Nn = N, EDn=ED, betan=beta, ST=ExternalSimutime)
    # np.savetxt(min_deviation_name, min_deviation)
    # ave_baseline_deviation_name = file_folder+"ave_baseline_deviation_N{Nn}ED{EDn}Beta{betan}Simu{ST}.txt".format(
    #     Nn = N, EDn=ED, betan=beta, ST=ExternalSimutime)
    # np.savetxt(ave_baseline_deviation_name, ave_baseline_deviation)
    length_geodesic_name = file_folder+"length_geodesic_N{Nn}ED{EDn}Beta{betan}Simu{ST}.txt".format(
        Nn = N, EDn=ED, betan=beta, ST=ExternalSimutime)
    np.savetxt(length_geodesic_name, length_geodesic)
    SPnodenum_vec_name = file_folder+"SPnodenum_N{Nn}ED{EDn}Beta{betan}Simu{ST}.txt".format(
        Nn = N, EDn=ED, betan=beta, ST=ExternalSimutime)
    np.savetxt(SPnodenum_vec_name, SPnodenum_vec, fmt="%i")
    nodepairs_for_eachgraph_vec_name = file_folder+"nodepairs_for_eachgraph_N{Nn}ED{EDn}Beta{betan}Simu{ST}.txt".format(
        Nn=N, EDn=ED, betan=beta, ST=ExternalSimutime)
    np.savetxt(nodepairs_for_eachgraph_vec_name, count_vec, fmt="%i")

    # max_dev_node_hopcount_name = "file_folder+"max_dev_node_hopcount_N{Nn}ED{EDn}Beta{betan}Simu{ST}.txt".format(
    #     Nn=N, EDn=ED, betan=beta, ST=ExternalSimutime)
    # np.savetxt(max_dev_node_hopcount_name, max_dev_node_hopcount, fmt="%i")
    #
    SPhopcount_name = file_folder+"min_SPhopcount_N{Nn}ED{EDn}Beta{betan}Simu{ST}.txt".format(
        Nn=N, EDn=ED, betan=beta, ST=ExternalSimutime)
    np.savetxt(SPhopcount_name, SP_hopcount, fmt="%i")
    #
    # min_dev_node_hopcount_name = "file_folder+"min_dev_node_hopcount_N{Nn}ED{EDn}Beta{betan}Simu{ST}.txt".format(
    #     Nn=N, EDn=ED, betan=beta, ST=ExternalSimutime)
    # np.savetxt(min_dev_node_hopcount_name, min_dev_node_hopcount, fmt="%i")


def distance_inlargeSRGG_realavg(N,ED,beta, ExternalSimutime):
    """
    :param N:
    :param ED:
    :param beta:
    ...
    nodepair_num = 10000
    unique_pairs = find_k_connected_node_pairs(G, nodepair_num)
    filefolder_name = "D:\\data\\geometric shortest path problem\\EuclideanSRGG\\max_min_ave_ran_deviation\\largenetwork\\realavg\\"
...
def distance_inSRGG_realavg(network_size_index, average_degree_index, beta_index, ExternalSimutime):
    Nvec = [22, 46, 100, 215, 464, 1000, 2154, 4642, 10000]
    kvec = [10]
    betavec = [128]
    ...
    # for small network, we generate 100 networks and selected all the node pair in the LCC
    if N > 100:
        distance_inlargeSRGG_realavg(N, ED, beta,ExternalSimutime)
    else:
        # Random select nodepair_num nodes in the largest connected component
        distance_insmallSRGG_realavg(N, ED, beta, ExternalSimutime)
...
def generate_SRGG_realavg(N, input_ED, beta):
