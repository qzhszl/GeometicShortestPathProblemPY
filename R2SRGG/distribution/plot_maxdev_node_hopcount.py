...
def plot_maxmindev_node_hocount_together_version2(N, ED, beta):
    ...
    ax1.tick_params(axis='y', labelsize=28)  # y 轴刻度字体大小
    # ax1.tick_params(axis='x', labelbottom = False)
    # ax1.set_xlabel(r'x', fontsize=24)
    ax1.set_ylabel(r'$f_X(\text{x})$', fontsize=30)
    ax1.set_xlabel(r'x', fontsize=30)
    # plt.xticks(fontsize=28)
    # plt.yticks(fontsize=28)

    ax1.text(0.5, 0.85, r'$h = 25$',
             transform=ax1.transAxes,
             fontsize=30,
             bbox=None)
    ...
    ax2.hist(values, bins=bins, alpha=0.7, color=colors[3], edgecolor=colors[3], density=True)  # 绘制直方图
    ax2.set_xticks(np.arange(min(values), max(values) + 1, 1))
    ax2.set_xticks([1, 7, 13, 19,25])
    ax2.set_yticks([0,0.1,0.2])
    ax2.yaxis.set_major_formatter(FormatStrFormatter('%.1f'))
    ax2.tick_params(axis="both", labelsize=28)  # x 轴刻度字体大小
    # ax2.set_ylabel(r'$f_X(\text{x})$', fontsize=24)
    ax2.set_xlabel(r'x', fontsize=30)
    # ax2.text(0.2, max(values) * 0.9, r'$h_{ij} = 25$', fontsize=20)
    ax2.text(0.2, 0.4, r'$N = 10^4$'+"\n" +r'$\beta = 4$'+ "\n"+ r"$\mathbb{E}[D] = 5$",
             transform=ax2.transAxes,
             fontsize=30,
             bbox=None)
    ...
    picname = "D:\\data\\geometric shortest path problem\\EuclideanSRGG\\max_min_ave_ran_deviation\\inpuavg_beta\\formaxhop\\distancetosinglenode\\maxdev_nodehop{Nn}ED{EDn}Beta{betan}Hop2425_v2.svg".format(
            Nn=N, EDn=ED, betan=beta, key=key)
