    ...
    if legendpara ==1:
        ...
        for text in leg.get_texts():
            if text.get_text() == 'Geo-based':  # 按名称匹配
                ...
                # text.set_color('red')  # （可选）改颜色
                # text.set_fontsize(14)  # （可选）改字体大小
...
def plot_predict_geodistance_Vs_random_withnoise_SP_R2(Edindex, betaindex, legendpara, theis_flag = False):
    # Figure 10 Appendix: AUPR
    # plot AUPRC/PR  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    N = 10000
    ED_list = [2, 5, 10, 100, 1000]  # Expected degrees
    ED = ED_list[Edindex]
    ...
    # print("lenpre", len(PrecisonRGG_specificnoise))
        ...
        for ExternalSimutime in range(10):
            try:
                precision_SRGG_Name = "D:\\data\\geometric shortest path problem\\EuclideanSRGG\\PredictionAUPR\\SRGG\\PRAUCSRGGED{EDn}Beta{betan}Noise{no}PYSimu{ST}.txt".format(
                    EDn=ED, betan=beta, no=noise_amplitude, ST=ExternalSimutime,no2=noise_amplitude)
                Precison_SRGG_5_times = np.loadtxt(precision_SRGG_Name)
                PrecisonSRGG_specificnoise.extend(Precison_SRGG_5_times)
                # PrecisonSRGG_specificnoise.append(Precison_SRGG_5_times)
            except FileNotFoundError:
                pass
        # nonzero_indices_geo = find_nonzero_indices(PrecisonRGG_specificnoise)
        # PrecisonRGG_specificnoise = list(filter(lambda x: not (math.isnan(x) if isinstance(x, float) else False), PrecisonRGG_specificnoise))
        ...
        for ExternalSimutime in range(10):
            try:
                precision_Geodis_Name = "D:\\data\\geometric shortest path problem\\EuclideanSRGG\\PredictionAUPR\\Geodistance\\PRAUCGeodisED{EDn}Beta{betan}Noise{no}PYSimu{ST}networksize{N}.txt".format(
        ...
        for ExternalSimutime in [0]:
            try:
                precision_Geodis_Name = "D:\\data\\geometric shortest path problem\\EuclideanSRGG\\PredictionAUPR\\Random\\PRAUCRandomED{EDn}Beta{betan}Noise{no}PYSimu{ST}networksize{N}.txt".format(
                    EDn=ED, betan=beta, no=noise_amplitude, ST=ExternalSimutime, N = N)
                PrecisonRandom_specificnoise = np.loadtxt(precision_Geodis_Name)
            except:
                pass
    ...
    # Plotting the bars
    # bar1 = ax.bar(x - 1.5*width, y1, width, label='RGG+net',yerr=(y_error_lower,RGG_precision_list_all_std), capsize=5, color=colors[3])
    bar2 = ax.bar(x-width, y2, width, label='SRGG+net', yerr=(y_error_lower,SRGG_precision_list_all_std), capsize=5, color=colors[0])
    bar3 = ax.bar(x, y3, width, label='Geo-based', yerr=(y_error_lower,Geo_precision_list_all_std), capsize=5, color=colors[2])
    bar4 = ax.bar(x + width, y4, width, label='Random', yerr=(y_error_lower, Random_precision_list_all_std), capsize=5,
                  color=colors[1])

    # Adding labels and title
    # ax.set_ylim(0.0005,2)
    ax.set_xlabel(r'Noise amplitude, $\alpha$', fontsize = 25)
    ax.set_ylabel('AUPR',fontsize = 25)
    ax.set_yscale("log")
    # title_name = "beta:{beta_n}, E[D]:{ed_n}".format(ed_n=ED, beta_n = beta)
    # ax.set_title(title_name)
    ax.set_xticks(x)
    ...
    if theis_flag:
        figname = "D:\\data\\geometric shortest path problem\\EuclideanSRGG\\PredictionAUPR\\AUPRGeoED{EDn}Beta{betan}N_withrandom.svg".format(
            EDn=ED, betan=beta)
    else:
        fignum_dict = {
            (5, 4): "a",
        ...
        figname = "D:\\data\\geometric shortest path problem\\EuclideanSRGG\\PredictionAUPR\\AUPRGeoSRGGRanED{EDn}Beta{betan}N_withrandom.svg".format(
            EDn=ED, betan=beta)
...
if __name__ == '__main__':
    ...
    # for Edindex in [0]:
    #     for betaindex in [2]:
    #         plot_predict_geodistance_Vs_random_withnoise_SP_R2(Edindex, betaindex, legendpara=1, theis_flag=False)
